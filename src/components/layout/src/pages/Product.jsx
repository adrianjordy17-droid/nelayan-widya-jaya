                ...
                <tbody>
                  {items.map((p, i) => {
                    const margin = (p.harga_jual != null && p.harga_modal != null)
                      ? p.harga_jual - p.harga_modal : null
                    return (
          ...
          <div style={card}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 22 }}>
              ...
              <button
                ...
                style={{ ...btnPrimary, background: (saving || !form.nama.trim()) ? '#c7c7cc' : '#0a84ff', cursor: (saving || !form.nama.trim()) ? 'not-allowed' : 'pointer' }}
              >
      ...
      {deleteConfirm && (
        <div style={overlay}>
          <div style={{ ...card, maxWidth: 320, textAlign: 'center', padding: '28px 24px' }}>
            ...
            <p style={{ fontSize: 13.5, color: '#8e8e93', margin: '0 0 20px' }}>Produk akan dihapus permanen dari daftar.</p>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setDeleteConfirm(null)} style={btnSecondary}>Batal</button>
              <button onClick={() => handleDelete(deleteConfirm)} style={{ ...btnPrimary, background: '#ff3b30' }}>Hapus</button>
            </div>
          </div>
        </div>
      )}
...
const btnSecondary = { flex: 1, padding: '11px 0', borderRadius: 11, background: '#f2f2f7', border: 'none', fontSize: 14, fontWeight: 600, color: '#3c3c43', cursor: 'pointer' }
const btnPrimary = { flex: 2, padding: '11px 0', borderRadius: 11, border: 'none', fontSize: 14, fontWeight: 600, color: 'white', cursor: 'pointer' }
