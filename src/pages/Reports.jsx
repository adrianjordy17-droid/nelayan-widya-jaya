...
export default function Reports() {
  ...
  // Period filter — pakai local time bukan UTC
  const now = new Date()
  const periodDocs = docs.filter(d => {
    if (!d.date) return true
    if (period === 'bulan ini') return d.date.startsWith(localYM(now))
    if (period === '3bulan') {
      const cut = new Date(now); cut.setMonth(cut.getMonth() - 3)
      return d.date >= localDate(cut)
    }
    if (period === '6bulan') {
  ...
  // Top products from period-filtered docs
  const productMap = {}
  ...
  function exportExcel() {
    ...
    // Sheet 3: Stock from products table
      ...
      {!loading && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
            {[
              { label: 'Total Penjualan', value: fmtShort(totalRevenue), sub: `omzet ${PERIOD_LABEL[period] || period}`, Icon: DollarSign,   iconColor: '#16a34a', iconBg: '#f0fdf4' },
